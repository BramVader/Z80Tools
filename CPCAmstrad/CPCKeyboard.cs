...
namespace CPCAmstrad
{
    public partial class CPCKeyboard : Form
    {
        ...
        private Queue<Action> keyActions = new Queue<Action>();
        private Queue<Action> delayedKeyActions = new Queue<Action>();
        ...
        private void HandleKeyDown(KeyInfo key, bool delayed)
        {
            ...
            if (delayed)
                delayedKeyActions.Enqueue(action);
            else
                keyActions.Enqueue(action);
        }
        ...
        {
            while (keyActions.TryDequeue(out Action action)) action();
            byte key = keyup[column];
            while (delayedKeyActions.TryDequeue(out Action action))
                keyActions.Enqueue(action);
            return key;
        }
    }
